...
var Speakerdetails = function (details) {
...
Speakerdetails.create = function (details, result) {
    let sql = `call  create_speaker(?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?);`;
    dbConn.query(
        sql,
        [
            details.title,
...
Speakerdetails.findById = function (id, result) {
  
    dbConn.query(
      "call Microsite_GetbyId_Speaker (?);",
      id,
      function (err, res) {
        if (err) {
          result(null, err);
  ...
  Speakerdetails.findAll = function (company_id, result) {
    let sql = `call  Microsite_Getll_Speaker();`;
  ...
  // Speakerdetails.delete = function (form_id, result) {
  //   dbConn.query("call  microsite_delete_speaker(?);", form_id, function (err, res) {
  //     if (err) {
  //       result(null, err);
  //     } else {
  //       result(null, res);
